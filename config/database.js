...
// We use sqlite3 or a lightweight embedded storage adapter
let dbInstance = null;
...
try {
  ...
  console.log('[DB] native sqlite3 module not installed yet. Using embedded memory/file storage fallback.');
}

// Fallback JSON-based SQLite engine if sqlite3 package is not built in environment
class FallbackDB {
  ...
  async run(sql, params = []) {
    ...
    if (sqlUpper.startsWith('INSERT INTO USERS')) {
      ...
      const newUser = {
        ...
        wallet_balance: params[5] !== undefined ? params[5] : 0,
        paid_status: params[6] !== undefined ? params[6] : 0,
        created_at: now
      };
    ...
    if (sqlUpper.startsWith('UPDATE USERS SET PAID_STATUS = 1')) {
  ...
  async get(sql, params = []) {
    ...
    if (sqlUpper.includes('COUNT(*) AS COUNT FROM USERS WHERE REFERRED_BY =')) {
      const id = params[0];
      const count = this.data.users.filter(u => u.referred_by === Number(id)).length;
      return { count };
    }
    ...
    if (sqlUpper.includes('SUM(COMMISSION_EARNED)')) {
      ...
      const matching = this.data.referrals.filter(r => r.referrer_id === Number(id) && (level ? r.level === Number(level) : true));
  ...
  async all(sql, params = []) {
    ...
    if (sqlUpper.includes('FROM TRANSACTIONS WHERE USER_ID =')) {
      const userId = params[0];
    ...
    if (sqlUpper.includes('FROM USERS WHERE REFERRED_BY =')) {
      const id = params[0];
      return this.data.users.filter(u => u.referred_by === Number(id));
    }
...
// Database Wrapper with SQLite + Fallback support
let dbEngine = null;
...
async function initDb() {
  if (dbEngine.isNative) {
    ...
    console.log('[DB] Fallback database initialized.');
  }
}
